...
3. **Прописать токен и chat в Vercel**
   ...
   **Блокировка спама по IP:** добавь переменную `BLOCKED_IPS` — список IP через запятую, без пробелов или с пробелами (например `5.143.9.148,1.2.3.4`). Запросы с этих IP будут получать 403, в Telegram ничего не уйдёт. После изменения переменной — Redeploy.
